...
const props = defineProps({
  max: { type: Number, default: 1000, required: true },
  min: { type: Number, default: 100, required: true },
  value: { type: Object as PropType<any>, required: true },
  isHorizontal: { type: Boolean, required: true },
});
const emit = defineEmits(["change", "input"]);
const rangeRound = (props.max - props.min) / 100;

const rangeSize = computed(() => ({
  get() {
    const valNum = Number(props.value);
    const size =
      valNum >= props.max
        ? props.max / rangeRound
        : valNum < props.min
        ? 0
        : (valNum - props.min) / rangeRound;
    return size;
  },
  set(val: number) {
    const valNum = val;
    const size =
      Number(val) === 0
        ? props.min
        : Math.floor((valNum / 100) * (props.max - props.min)) + props.min;
