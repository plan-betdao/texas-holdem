      ...
      >
        ...
        <div class="sit-player" v-if="sit.player">
          <div class="player" :class="{ fold: sit.player.status === -1 }">
            <div class="count-down" v-show="actionUserId === sit.player.userId">
              {{ time }}
            </div>
            ...
            <div
              ...
              :class="{
                ...
                'close-time-out':
                  time > 0 && time < 10 && actionUserId === sit.player.userId,
              }"
              v-show="
                sit.player.counter >= 0 || sit.player.actionCommand === 'allin'
              "
            >
          ...
          <div class="cards" v-show="showHandCard(sit)">
            <div class="hand-card">
              ...
              <div
                ...
                v-show="
                  time < 15 &&
                  sit.player.delayCount > 0 &&
                  actionUserId === sit.player.userId
                "
                @click="delayTime"
              >
            ...
            <div class="ready" v-show="handCard && handCard.length === 0">
              ready
            </div>
...
import type { ILinkNode } from "@/utils/Link";
import type { ISit } from "@/interface/ISit";
...
const props = defineProps({
  ...
  currPlayer: { type: IPlayer, required: true },
  commonCard: { type: Array as PropType<string[]>, required: true },
  sitLink: { type: ILinkNode as PropType<ILinkNode<ISit>>, required: true },
  handCard: { type: Array as PropType<string[]>, required: true },
  winner: { type: IPlayer as PropType<IPlayer[][]>, required: true },
  isPlay: { type: Boolean, required: true },
  roomConfig: { type: IRoom, required: true },
  actionUserId: { type: String, required: true },
  valueCards: { type: String, required: true },
  time: { type: Number, default: 30, required: true },
});

const emit = defineEmits(["buyIn", "delay"]);
...
function buyIn(size: number) {
  showBuyIn = false;
  this.currPlayer.counter += size;
  emit("buyIn", Number(size));
  sitDown(this.currSit);
}

function showHandCard(sit: ISit) {
  return sit.player?.userId === this.currPlayer?.userId;
}

function PokeStyle(cards: string[]) {
